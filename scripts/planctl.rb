...
class PlanCtl
  ...
  def status(format:)
    ...
    when 'json'
      ...
      puts 'Phase-AI-Anime plan state'
  ...
  def git_guard_message
    ...
    lines << '[planctl] Phase-AI-Anime Workflow relies on git for phase-level whitelist diffing, rollback, and handoff verification.'
  ...
  def render_resolve(result, format)
    ...
    when 'paths'
      ...
      puts 'Phase-AI-Anime phase context'
  ...
  def render_handoff(snapshot, format)
    ...
    when 'json'
      ...
      puts 'Phase-AI-Anime execution handoff'
      puts "State file: #{snapshot['state_file']}"
      puts "Handoff file: #{snapshot['handoff_file']}"
      puts "Updated at: #{snapshot['updated_at'] || 'not recorded yet'}"
  ...
  def handoff_markdown(snapshot)
    lines = []
    lines << '# Phase-AI-Anime Execution Handoff'
    lines << ''
  ...
  def build_finalize_recommendations(git_state, delivery_gate, health, phase_rows)
    ...
    recs << '跑一次成片链路验收：单 phase 只覆盖局部，最后必须检查剧集承诺、角色一致性、镜头连续性、音画同步、生成任务和合成清单是否在整条链路上成立。'
    recs << '组织一次人工审片：把里程碑 commit 链 + plan/state.yaml + plan/handoff.md + plan/phases + anime/assembly 作为审计材料，让至少一位非本轮执行者检查。'
    recs << '决定如何发布：若本轮产出对应可公开版本，可运行 `git tag -a video-vX.Y.Z -m "..."` 记录成片节点；若暂不发布，就在交接文档里写清楚“此次未公开”的理由。'
    recs << '把 plan/ 归档：保留作为复盘材料；若同一仓库还要继续下一条漫剧或下一轮制作，先 `git mv plan plan-archive-<date>` 再重跑本 Skill 生成新 plan，避免污染当前 manifest。'
    recs << '写一份制作交接说明 / 复盘：说明本轮改了什么、为何这样改、当前留下哪些角色、镜头、音频、生成或合成风险。'
    recs << '与人类决策点对齐：是否上线、是否公开发布、是否商用、是否安排长期维护或进入下一轮制作。AI 不要自行决定这些；finalize 输出仅作为决策素材。'

    recs
  end

  def render_finalize_dashboard(d)
    puts '=== Phase-AI-Anime Final Execution Dashboard ==='
    ...
    if delivery['enabled']
      ...
      puts "Current delivery files: #{delivery.dig('metrics', 'file_count')}"
      puts "Current total chars: #{delivery.dig('metrics', 'total_chars')}"
      puts "Current unit count: #{delivery.dig('metrics', 'chapter_count')} (pattern: #{delivery.dig('targets', 'chapter_pattern')})"
      ...
      puts "Target units: min=#{chapter_target['min'] || '-'} max=#{chapter_target['max'] || '-'}"
  ...
  def project_delivery_issues(metrics)
    ...
    if length_target.is_a?(Hash)
      ...
      issues << "current delivery length #{total} chars is below target minimum #{min}" if min && total < min
      issues << "current delivery length #{total} chars exceeds target maximum #{max}" if max && total > max
    end
    ...
    if chapter_target.is_a?(Hash)
      ...
      issues << "current unit count #{count} is below target minimum #{min}" if min && count < min
      issues << "current unit count #{count} exceeds target maximum #{max}" if max && count > max
    end
  ...
  def project_delivery_notes(metrics)
    ...
    notes << "current delivery files: #{metrics['file_count']}" if metrics['file_count']
  ...
  def legacy_manifest_migration_notes
    ...
    if story_phases.any? && !repo_policy_explicitly_configured?
      notes << "legacy story manifest is missing repo_policy; add repo_policy.mode (usually standalone) before the next major production run. Story phase(s): #{story_phases.map { |phase| phase['id'] }.join(', ')}"
    end
    if draft_phases.any? && !project_profile_explicitly_configured?
      notes << "legacy delivery-bearing manifest is missing project_profile; add delivery_tier, delivery_paths, target_length_chars, and target_chapters to enable delivery gates. Delivery phase(s): #{draft_phases.map { |phase| phase['id'] }.join(', ')}"
    end
