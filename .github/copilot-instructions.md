# phase-ai-anime-skill Phase 执行规约

> **适用范围**：本文件同时作为 GitHub Copilot、Claude Code、Codex / 通用 Agent 的会话级强制约束，内容必须与 `.github/copilot-instructions.md`、`CLAUDE.md`、`AGENTS.md` 三份字节一致。
...
2. 若是连续推进，读取 `plan/handoff.md`，再运行 `ruby scripts/planctl.rb advance --strict`。
3. 若用户指定 phase，运行 `ruby scripts/planctl.rb resolve <phase-id> --format prompt --strict`。
...
```bash
ruby scripts/planctl.rb finalize
```
